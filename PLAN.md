...
- [ ] **2.20** `VALUES (expr_list) [, …]` as a select body
- [ ] **2.21** `INSERT … SELECT` (read-path INSERT from query)
- [ ] **2.22** `INSERT … DEFAULT VALUES`
...
- [ ] **5.3.1** B-tree page merging on delete (when page is too empty, redistribute or merge with sibling)
...
- [ ] **6.7** Codegen: aggregate without GROUP BY (single-row result for `SELECT count(*) FROM t`)
- [ ] **6.8** Codegen: `GROUP BY` + `ORDER BY` — two-pass (aggregate then sort result)
- [ ] **6.9** `NULL` handling in aggregates (NULL group keys, NULL exclusion from `sum`/`avg`, `count(*)` vs `count(col)`)
...
- [ ] **11.6** Built-in window functions: `lead(expr [, offset [, default]])`, `lag(expr [, offset [, default]])`
...
- [ ] **13.1** WAL file format: `-wal` header (magic, version, page size, checkpoint sequence, salt, checksum)
