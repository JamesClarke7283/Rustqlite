...
use rustqlite::format::TextEncoding;
...
fn cmd_open(state: &mut ShellState, args: &[String]) {
    ...
    match rustqlite::sqlite3_open(path) {
        Ok(conn) => {
...
fn print_version() {
    ...
    println!("(rustqlite — a faithful Rust reimplementation)");
}
...
.version               Show the SQLite version targeted by rustqlite"
    );
}
