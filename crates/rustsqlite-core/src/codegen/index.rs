...
pub fn compile_create_index(
    ...
    let ctx = super::expr::Ctx { table, cursor: table_cursor, register_base: None, join_tables: None, index_read: None, subquery_resolver: None };
    ...
    for (i, ic) in dummy.columns.iter().enumerate() {
        ...
        if let Some(expr) = &ic.expr {
            super::expr::compile_expr(&mut b,
                expr,
                target,
                super::expr::Ctx { table, cursor: table_cursor, register_base: None, join_tables: None, index_read: None, subquery_resolver: None },
            )?;
        } else {
